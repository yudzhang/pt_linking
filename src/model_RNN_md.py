...
class MotionRNN(object):
    def __init__(self, ninput, noutput, nclasses, lstms, lstmo, R, H, L, k,
        ...
        with tf.variable_scope('rnnx'):
            ...
            x_fc = tf.layers.Dense( self._L, activation=None)(self.x)
            if rnn == 'lstm':
            ...
            if self._isAtt:
                ...
                x_output = tf.layers.Flatten()(x_all_output)
            if self._R == 1:
                if rnn == 'lstm':
        ...
        with tf.variable_scope('fcy'):
            ...
            y_fc = tf.layers.Dense( self._L, activation=None)(self.y)
...
class MotionCRNN(object):
    def __init__(self, ninput, noutput, nclasses, lstms, lstmo, R, H, L, k,
        ...
        with tf.variable_scope('rnn'):
            fc = tf.layers.Dense( self._L, activation=None)(fx)
            if rnn == 'lstm':
        ...
        with tf.variable_scope('fcy'):
            # define fully connected layer for t+1 displacement vector
            y_fc = tf.layers.Dense( self._L, activation=None)(fy)
            y_dp = tf.layers.Dropout( self._dropout)(y_fc)
            y_fc = tf.layers.Dense( self._L, activation=act)(y_dp)
            y_dp = tf.layers.Dropout( self._dropout)(y_fc)
            y_output = tf.layers.Flatten()(y_dp)
...
class MotionRRNN(object):
    def __init__(self, ninput, noutput, nclasses, lstms, lstmo, R, H, L, k,
        ...
        with tf.variable_scope('rnnx'):
            # define cells for rnn and dropout between layers
            fxrnn = tf.layers.Dense( self._L, activation=None)(fx)
            if rnn == 'lstm':
                cell = tf.nn.rnn_cell.LSTMCell(self._L, activation=act)
            elif rnn == 'gru':
                cell = tf.nn.rnn_cell.GRUCell(self._L, activation=act)
...
class MotionRHRNN(object):
    def __init__(self, ninput, ninputp, noutput, nclasses, lstms, lstmo, R, H, L, k,
        ...
        with tf.variable_scope('fcy'):
            ...
            y_fc = tf.layers.Dense( self._L, activation=None)(y_con)
            y_dp = tf.layers.Dropout( self._dropout)(y_fc)
            y_fc = tf.layers.Dense( self._L, activation=act)(y_dp)
            y_dp = tf.layers.Dropout( self._dropout)(y_fc)
            y_output = tf.layers.Flatten()(y_dp)
...
# Alation exp——loss part 3loss combination randomly—— no need
