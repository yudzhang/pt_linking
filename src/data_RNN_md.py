...
import sys
sys.path.append('./')
from convert_XML import writeXML, writeTXT
...
def train_test(scenario, s, out, iter, istrain='tr', togit=0, dodelete=False, isspt=True):
    ...
    if istrain == 'train':
        ...
        x_t = loaded['x'] # (n,3,10)3个位置一组，每个位置有10个特征，最后两个是位置
        y_t = loaded['y'] # (n,1,10)不知道为什么全都是 00000...-1 -1
        t_t = loaded['t'] # (n,2)   是x的下一时刻的坐标
        p_t = loaded['p'] # (n,2)   全都是 [0,1] 估计是二分类label
        l_t = loaded['l'] # (n,2)   x最后一个位置的坐标
        d_t = loaded['d'] # (n,)    全0 估计是匹配cost
        loaded = np.load(pathsave + '/after comb/' + 'ax_F' + str(s) + str(out) + '.npz')
        x_f = loaded['x'] # (n,3,10) 同上
        y_f = loaded['y'] # (n,1,10) 假的下一位置的向量10
        t_f = loaded['t'] # (n,2)    真实的下一时刻的坐标
        p_f = loaded['p'] # (n,2)    全部都是[1,0] 二分类label
        l_f = loaded['l'] # (n,2)    x最后一个位置的坐标
        d_f = loaded['d'] # (n,)     都是浮点数，2,3,4,5，6
        #将正样本的乱序
        permutation = np.random.permutation(p_t.shape[0]) #将正样本的乱序
        x_t = x_t[permutation]
        y_t = y_t[permutation]
        t_t = t_t[permutation]
        p_t = p_t[permutation]
        ...
        # train datset的均值方差 10个维度，每个维度得到均值和方差，包括linked x 和 candidate y
